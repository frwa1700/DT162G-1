...
router.post('/', function(req, res, next){
    if (req.body)
        collection.insertOne(req.body, function (err, result) {
            if (err) {
                console.log('Posting customer failed. Data is:' + req.body);
                res.status(400).send("error : \"Posting customer failed\"");
            } else {
                console.log('Insert new customer successful.')
                res.status(200).send("success : \"Posted customer successfully\"");
            }
        })
